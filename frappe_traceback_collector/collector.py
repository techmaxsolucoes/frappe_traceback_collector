...
def install_collector():
	"""Replace the default except hook in sys to enable the automation of ticket collector"""
	def collector(type, exception, traceback):
		collect(exception)
	
	if 'frappe_traceback_collector' in frappe.get_installed_apps():
		sys.excepthook = collector
		print "Traceback collector running ..."
